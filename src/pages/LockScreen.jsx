  ...
  return (
    <div className="login-scene" style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="login-card" style={{ fontFamily: 'Playfair Display, serif', letterSpacing: 1, zIndex: 2, position: 'relative' }}>
        <h1 style={{ fontFamily: 'Playfair Display, serif', fontWeight: 700, color: 'var(--title-color)', fontSize: '2rem', marginBottom: 8 }}>
          Hola <span style={{ transition: 'color 0.5s', color: 'var(--text-subtle)' }}>{greetings[greetingIdx]}</span>...
        </h1>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', margin: '20px 0' }}>
          <input
            ...
            style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.1rem', borderRadius: 14, border: '2px solid var(--accent-alt)', padding: '14px 16px', width: 180, color: 'var(--text-main)' }}
          />
        ...
        <p className="hint" style={{ color: 'var(--text-subtle)', fontFamily: 'Playfair Display, serif', fontSize: '1rem', marginTop: 10 }}>{hint}</p>
      </div>
    </div>
  )
}
